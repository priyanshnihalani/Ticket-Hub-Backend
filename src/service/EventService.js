...
class EventService {
    ...
    async getEventById(id) {
        try {
            ...
            if (!result) throw new InvalidRequestException(MessageConstant.NO_DATA_FOUND)
    ...
    async updateEventById(id, data) {
        try {
            ...
            const [, updatedResult] = await EventRepository.updateEventById(id, data);
    ...
    async getAllEvent(isAdminEvent) {
        try {
            const result = await EventRepository.getAllEvent(isAdminEvent);
            return { success: true, body: result }
        } catch (error) {
            throw error
        }
    ...
    async getEventListByFilterSort(req) {
        try {
            ...
            const result = await EventRepository.getEventListByFilterSort(req?.filter, req?.sort, req?.page);
            return { success: true, body: result }
        } catch (error) {
            throw error
        }
